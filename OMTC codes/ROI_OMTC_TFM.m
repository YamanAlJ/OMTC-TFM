...
PC3_ARRAY = [ "\PC3AR_10nMR1881_3kPa"]   

  
             
%USER INPUT 2/8:TFM data array name files            

PC3_TFM = [ "\PC3AR_10nMR1881_3kPa"]
...
TFM_ATTRIBUTE = 2; % USER INPUT 3/8:to calculate ROI RMST input 1, Strain Energy input2
factor = 2; % same as TFM_ATTRIBUTE
...
CONDITION_OMTC_FINAL = 'D:\RAW DATA\EH_OMTCTFM_PC3AR_3kPa_10nMR1881_mOrange_48h_HEAT_06102022'
%USER INPUT 7/8: directory name where TFM results are located
CONDITION_TFM_FINAL = 'D:\RAW DATA\EH_OMTCTFM_PC3AR_3kPa_10nMR1881_mOrange_48h_HEAT_06102022'
...
%           LINE OF BEST FIT  
            ...
            hold on
            leg = legend( '10nM R1881','','Location',location);
            title (leg, 'Media' );
            ylim([0,1000]);
        end
...
function [conditionStrainArray,conditionModulusArray] = ROISEAM (OMTC_FOLDER, TFM_FOLDER,EXPAND_MARGIN, TFM_ATTRIBUTE,G_limit,CONDITION_OMTC_FINAL,CONDITION_TFM_FINAL,factor)
    ...
    OMTC_CONDITION_DIR = strcat(CONDITION_OMTC_FINAL, OMTC_FOLDER);
    ROICoordsStruc = dir(strcat(OMTC_CONDITION_DIR, "/series_*/RoiCoords.txt"));
...
function [WINDOW_SIZE, DEDRIFTED_SIZE, ps1, step_size] = importResultsMatValues(TFM_FOLDER,CONDITION_TFM_FINAL)
    ...
    TFM_CONDITION_DIR = strcat(CONDITION_TFM_FINAL, TFM_FOLDER);
    resultsFileString = strcat(TFM_CONDITION_DIR, '/results.mat');
...
function [u,v,tx,ty,series] = importDispTracArrays(TFM_FOLDER, RoiCoordFileString, OMTC_CONDITION_DIR,CONDITION_TFM_FINAL)
    ...
    TFMSeriesFolder = strcat(TFM_CONDITION_DIR, '/series_', series, "/Series", series);
    
    TFMDisplacementString = strcat(TFMSeriesFolder, '/Displacement/d_arrays_corrected/trim_displacement_mat001.mat');
    TFMTractionString = strcat(TFMSeriesFolder, '/Traction/t_arrays_corrected/trim_traction_mat001.mat');
...
function [strain_array, AM_array] = ExtractRoiValues(roi_array,EXPAND_MARGIN,crop_offset, u, v, tx, ty, ps1, step_size,series, Dictionary,KeySet, TFM_ATTRIBUTE,G_limit,factor)
    ...
    for j = 1:4:height(roi_array);
       ...
       % 4 - x,y of bottom left corner
        roix1 = roi_array{j,4};      % pixel coord of ROI's left side
        roix2 = roi_array{j+1,4};    % pixel coord of ROI's right side
        roiy3 = roi_array{j,6};      % pixel coord of ROI's top side
        roiy4 = roi_array{j+2,6};    % pixel coord of ROI's bottom side
...
function [AMDictionary, keySet] = CreateApparentModulusDictionary(OMTC_CONDITION_DIR,OMTC_FOLDER) 
    ...
    AMTableString = OMTC_CONDITION_DIR + "/" + condition + "_FINAL/" + condition + "_SUMMARY_STATS_FINAL_GM.xlsx";
	...
	for ii = 1:height(AMTable) 
        ...
        keySet = [keySet AMKey];
        valueSet =[valueSet table2cell(AMTable(ii,6))];%Apparent Modulus
...
function series = parseOutSeries (FileString,OMTC_CONDITION_DIR)
    series = extractBetween(FileString, "ser", "/");
    series = extractAfter(series, "ies_")
end